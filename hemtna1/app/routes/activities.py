...
def get_activities():
    ...
    child_counts = {}
    for a in activities:
        if a.child_name:  # فقط إذا كان هناك اسم طفل
...
def add_activity():
    ...
    activity = Activity(
        ...
        child_name=data.get("child_name"),  # أصبح اختياري
        doctor_id=safe_int(data.get("doctor_id")) if data.get("doctor_id") else None,  # أصبح اختياري
        parent_id=safe_int(data.get("parent_id")) if data.get("parent_id") else None  # أصبح اختياري
    )
